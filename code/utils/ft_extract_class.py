...
class SubmoveData2Feat:
    ...
    acc_triax: np.ndarray = field(default_factory=lambda: np.array([]))
    ...
    def run_mean_acc(self):
        value = acc_fts.get_mean_acc(self)
        # normalize to length of submovement data
        value = value / len(self.acc_svm)
        return value
    
    def run_peak_acc(self):
        value = acc_fts.get_peak_acc(self)
        return value
    
    def run_mav_per_axis(self):
        value = acc_fts.get_mav_per_axis(self)
        value = np.mean(value)  # mean of 3 axes
        return value
    ...
    def run_jerk_rms(self):
        value = acc_fts.get_jerk_rms(self=self)
        return value
    
    def run_zero_crossing_rate(self):
        value = acc_fts.get_zero_crossing_rate(self=self)
        return value
    
    def run_approx_entropy(self):
        value = acc_fts.get_approx_entropy(self=self)
        return value
    
    def run_autocorr_lag1(self):
        value = acc_fts.get_autocorr_lag1(self=self)
        return value
    
    def run_kurtosis(self):
        value = acc_fts.get_kurtosis(self=self)
        return value
    
    def run_skewness(self):
        value = acc_fts.get_skewness(self=self)
        return value
    
    def run_axis_correlations(self):
        value = acc_fts.get_axis_correlations(self=self)
        return value
    
    def run_svm_var_to_gravity_var_ratio(self):
        value = acc_fts.get_svm_var_to_gravity_var_ratio(self=self)
        if np.isnan(value): value = 0
        return value
    ...
    def run_spectral_entropy(self):
        value = acc_fts.get_spectral_entropy(self)
        return value
    
    def run_spectral_edge_freq(self):
        value = acc_fts.get_spectral_edge_freq(self)
        return value
